...
def iqindportero(df, j1, equipo, pos):
    ...
    df['Long balls per pass']=df['Long balls total']/df['Open Play Passes']
    df['% pases son largos']=df['Long balls per pass'].rank(pct=True)
    df['Goles parados']=df['Goals Saved Above Average'].rank(pct=True)
    ...
    df['Pases']=df['Open Play Passes'].rank(pct=True)
...
def iqindcentral(df, j1, equipo, pos):
    ...
    df['Asistencias Jugada Abierta p90']=df['Open Play Assists'].rank(pct=True)
    df['Promedio minutos jugados por partido']=df['Minutes'].rank(pct=True)
    df['Pases hacia atrás %']=df['Pass Bwd%'].rank(pct=True)
    df['Pases hacia adelante %']=df['Pass Fwd%'].rank(pct=True)
    ...
    df['Éxito 1vs1 defensivo']=df['Tackle/Dribble Past%'].rank(pct=True)
    ...
    df['Pases hacia atrás 3/3 %']=df['Open Play Pass Bwd% A⅓'].rank(pct=True)
    df['Pases hacia adelante 3/3 %']=df['Open Play Pass Fwd% A⅓'].rank(pct=True)
    df['Pases último tercio']=df['Open Play Passes A⅓'].rank(pct=True)
    df['Balones recuperados campo rival']=df['Ball Recoveries A½'].rank(pct=True)
    ...
    df['Tiros']=df['Non Penalty Shots'].rank(pct=True)
    df['xG']=df['Non Penalty xG'].rank(pct=True)
    df['xG por tiro']=df['Non Penalty xG/Shot'].rank(pct=True)
    df['Goles (sin penales)']=df['Non Penalty Goals'].rank(pct=True)
    df['Contribución de gol']=df['Non Penalty xG & xG Assisted'].rank(pct=True)
    df['xG post-tiro']=df['Non Penalty PSxG'].rank(pct=True)
    df['OBV total']=df['OBV'].rank(pct=True)
    df['OBV pases']=df['Pass OBV'].rank(pct=True)
    df['OBV tiros']=df['Shot OBV'].rank(pct=True)
    df['OBV acciones defensivas']=df['Defensive Action OBV'].rank(pct=True)
    df['OBV conducciones y regates']=df['Dribble & Carry OBV'].rank(pct=True)
    df['OBV portero']=df['Goalkeeper OBV'].rank(pct=True)
    ...
    df['Pases al área']=df['Open Play Passes Into Box'].rank(pct=True)
    df['% pases bajo presión']=df['Passes Pressured%'].rank(pct=True)
    df['Éxito pases']=df['Passing%'].rank(pct=True)
    df['xA']=df['Open Play xG Assisted'].rank(pct=True)
    ...
    df['Pases filtrados']=df['Through Balls'].rank(pct=True)
    ...
    df['Presiones cancha rival']=df['Pressures A½'].rank(pct=True)
    df['PS xG vs xG']=df['Non Penalty PSxG']/df['Non Penalty xG']
...
def iqindlateral(df, j1, equipo, pos):
    ...
    df['Press %']=df[df['Primary Position'] == 'Lateral']['Pressure Regains']/df[df['Primary Position'] == 'Lateral']['Pressures']
    df['Éxito duelos aéreos']=df[df['Primary Position'] == 'Central']['Aerial Win%'].rank(pct=True)
    df['Duelos aéreos ganados']=df['Aerial Wins'].rank(pct=True)
    df['Acciones agresivas']=df['Aggressive Actions'].rank(pct=True)
    df['Asistencias Torneo']=df['Assists'].rank(pct=True)
...
def iqindcontencion(df, j1, equipo, pos):
    ...
    df['Press %']=df[df['Primary Position'] == 'Contención']['Pressure Regains']/df[df['Primary Position'] == 'Contención']['Pressures']
    df['Éxito duelos aéreos']=df[df['Primary Position'] == 'Central']['Aerial Win%'].rank(pct=True)
    df['Duelos aéreos ganados']=df['Aerial Wins'].rank(pct=True)
    df['Acciones agresivas']=df['Aggressive Actions'].rank(pct=True)
    df['Asistencias Torneo']=df['Assists'].rank(pct=True)
...
def iqindvolante(df, j1, equipo, pos):
    ...
    df['Press %']=df[df['Primary Position'] == 'Extremo']['Pressure Regains']/df[df['Primary Position'] == 'Extremo']['Pressures']
    df['Éxito duelos aéreos']=df[df['Primary Position'] == 'Central']['Aerial Win%'].rank(pct=True)
    df['Duelos aéreos ganados']=df['Aerial Wins'].rank(pct=True)
    df['Acciones agresivas']=df['Aggressive Actions'].rank(pct=True)
    df['Asistencias Torneo']=df['Assists'].rank(pct=True)
...
def iqinddelantero(df, j1, equipo, pos):
    ...
    df['Press %']=df[df['Primary Position'] == 'Delantera']['Pressure Regains']/df[df['Primary Position'] == 'Delantera']['Pressures']
    df['Éxito duelos aéreos']=df[df['Primary Position'] == 'Central']['Aerial Win%'].rank(pct=True)
    df['Duelos aéreos ganados']=df['Aerial Wins'].rank(pct=True)
    df['Acciones agresivas']=df['Aggressive Actions'].rank(pct=True)
    df['Asistencias Torneo']=df['Assists'].rank(pct=True)
